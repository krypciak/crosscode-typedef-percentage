...
const repoPath = `/home/krypek/Programming/crosscode/ultimate-crosscode-typedefs`
...
const gen_root: Namespace = { startLine: 0, endLine: sp.length, name: '', path: '', indentLevel: 0, classes: {}, fields: new Set(), functions: new Set(), namespaces: {} }
...
let li: number = 0
const mod_root: Namespace = { startLine: 0, endLine: sp.length, name: '', path: '', indentLevel: 0, classes: {}, fields: new Set(), functions: new Set(), namespaces: {} }
...
function compare(prim: Namespace, seco: Namespace) {
    const primClasses = getClasses(prim)
    const primFields = getFields(prim)
    const primFunctions = getFunctions(prim)

    const secoClasses = getClasses(seco)
    const secoFields = getFields(seco)
    const secoFunctions = getFunctions(seco)

    const commonClasses = intersect(primClasses, secoClasses)
